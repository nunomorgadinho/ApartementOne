...
package {
	...
	public class Collision {
		...
		/**
		 * 
		 * @param target1 - The instance 1 to check for collision.
		 * @param target2 - The instance 2 to check for collision.
		 * @param tollerance - a number from 0 to 255 that specifies the transparency tolerance 
		 * when testing the collision. A higher number will increase the tolerance 
		 * (ie. allow more transparent parts of the MovieClip to register collisions). Defaults to 0.
		 * 
		 * @return 
		 * 
		 */		
		public static function getCollision(target1:DisplayObject, target2:DisplayObject, tollerance:Number=0):Rectangle 
		{
			...
			if (target1.parent==target2.parent) 
			{
				...
				if (isIntersecting) 
				{
					...
					var alpha1:BitmapData = Collision.getAlphaMap(target1, combinedrect, BitmapDataChannel.RED, rect1);
					var alpha2:BitmapData = Collision.getAlphaMap(target2, combinedrect, BitmapDataChannel.GREEN, rect2);
		 ...
		 * 
		 */		
		public static function getAlphaMap(target:DisplayObject,rect:Rectangle,channel:uint,myrect:Rectangle):BitmapData 
		{
